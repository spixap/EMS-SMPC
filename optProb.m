...
if simIter == 1
    ...
    % GT Model
    P_gt           = optimvar('Power_GT',par.N_prd,par.N_scn,par.N_gt,'LowerBound',0,'UpperBound',par.P_gt_max);
    % Fuel Model
    pwl_fuel_w     = optimvar('fuel_PWA_weight',par.N_prd,par.N_scn,par.N_gt,par.N_pwl,'LowerBound',0);
    pwl_fuel_bin   = optimvar('fuel_PWA_indicator',par.N_prd,par.N_scn,par.N_gt,par.N_pwl,'Type','integer','LowerBound',0,'UpperBound',1);
    % Degradation Model
    pwl_deg_w      = optimvar('degradation_PWA_weight',par.N_prd,par.N_scn,par.N_pwl,'LowerBound',0);                                     % corresponds to \Delta_{t,m}
    pwl_deg_bin    = optimvar('degradation_PWA_indicator',par.N_prd,par.N_scn,par.N_pwl,'Type','integer','LowerBound',0,'UpperBound',1);   % corresponds to \delta_{t,m} interval indicator
    deg            = optimvar('degradation',par.N_prd,par.N_scn,'LowerBound',0);                % (Y from PWA)     corresponds to deg_{t}
    deg_cyc        = optimvar('dummy_cyclic_degradation',par.N_prd,par.N_scn,'LowerBound',0);            % corresponds to DPcy'_{t}
    DP_cyc         = optimvar('Total_cyclic_degradation',par.N_scn,'LowerBound',0);                   % corresponds to DPcy
    %         DP_cal         = optimvar('Total_calendar_degradation',par.N_scn,'LowerBound',0);                 % corresponds to DPshelf
    DP             = optimvar('Total_degradation',par.N_scn,'LowerBound',0);                          % corresponds to DP
    % Battery Model
    bin_dis       = optimvar('storage_discharge_indicator',par.N_prd,par.N_scn,'Type','integer','LowerBound',0,'UpperBound',1); % 1: DIS-charging
    % Dump (slack vriable for over-generation)
    P_d           = optimvar('Power_dump',par.N_prd,par.N_scn,'LowerBound',0,'UpperBound',Inf); % this acts as a sluck variable for feasibility of the solution
    % Final SoC penalization (slack vriable for over-generation)
    soc_dev_abs   = optimvar('absolute_SoC_deviation_from_reference',par.N_scn,'LowerBound',0);
    soc_dev_p     = optimvar('positive_slack_SoC_deviation',par.N_scn,'LowerBound',0);
    soc_dev_n     = optimvar('negative_slack_SoC_deviation',par.N_scn,'LowerBound',0);
    
    % States x
    DOD            = optimvar('DoD',par.N_prd,par.N_scn,'LowerBound',0,'UpperBound',1); % (X from PWA)     corresponds to 1-SoC_{t} % | state 1-x
    bin_gt         = optimvar('GT_status_indicator',par.N_prd,par.N_scn,par.N_gt,'Type','integer','LowerBound',0,'UpperBound',1);
    % Actuation u
    P_ch           = optimvar('Power_charging',par.N_prd,par.N_scn,'LowerBound',0,'UpperBound',par.P_bat_max);
    P_dis          = optimvar('Power_discharging',par.N_prd,par.N_scn,'LowerBound',0,'UpperBound',par.P_bat_max);
    bin_strUP_gt   = optimvar('GT_startUP_indicator',par.N_prd,par.N_scn,par.N_gt,'Type','integer','LowerBound',0,'UpperBound',1);
    bin_shtDOWN_gt = optimvar('GT_shutDOWN_indicator',par.N_prd,par.N_scn,par.N_gt,'Type','integer','LowerBound',0,'UpperBound',1);
    ...
    P_gt_k0           = optimvar('Power_GT_k0',par.N_gt,'LowerBound',0,'UpperBound',par.P_gt_max);
    bin_strUP_gt_k0   = optimvar('GT_startUP_indicator_k0',par.N_gt,'Type','integer','LowerBound',0,'UpperBound',1);
    ...
    j = 1;
    while j <= par.N_prd + par.N_scn + par.N_gt + par.N_pwl
        while j<= par.N_prd
            indexNames{j} = append('k+',int2str(j-1));
            j=j+1;
        end
        while j<= par.N_prd + par.N_scn
            indexNames{j} = append('scen_',int2str(j-par.N_prd));
            j=j+1;
        end
        while j<= par.N_prd + par.N_scn + par.N_gt
            indexNames{j} = append('GT_',int2str(j-par.N_prd-par.N_scn));
            j=j+1;
        end
        while j<= par.N_prd + par.N_scn + par.N_gt + par.N_pwl
            indexNames{j} = append('point_',int2str(j-par.N_prd-par.N_scn-par.N_gt));
            j=j+1;
        end
    end
    ...
    for i = 1:4
        if i==1
            pwl_fuel_w.IndexNames{i}     = indexNames(1,1:par.N_prd);
            pwl_fuel_bin.IndexNames{i}   = indexNames(1,1:par.N_prd);
            P_gt.IndexNames{i}           = indexNames(1,1:par.N_prd);
            bin_gt.IndexNames{i}         = indexNames(1,1:par.N_prd);
            bin_strUP_gt.IndexNames{i}   = indexNames(1,1:par.N_prd);
            bin_shtDOWN_gt.IndexNames{i} = indexNames(1,1:par.N_prd);
            pwl_deg_w.IndexNames{i}      = indexNames(1,1:par.N_prd);
            pwl_deg_bin.IndexNames{i}    = indexNames(1,1:par.N_prd);
            deg.IndexNames{i}            = indexNames(1,1:par.N_prd);
            DOD.IndexNames{i}            = indexNames(1,1:par.N_prd);
            deg_cyc.IndexNames{i}        = indexNames(1,1:par.N_prd);
            bin_dis.IndexNames{i}        = indexNames(1,1:par.N_prd);
            P_ch.IndexNames{i}           = indexNames(1,1:par.N_prd);
            P_dis.IndexNames{i}          = indexNames(1,1:par.N_prd);
            P_d.IndexNames{i}            = indexNames(1,1:par.N_prd);
            
            DP_cyc.IndexNames{i}         = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            DP_cal.IndexNames{i}         = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            DP.IndexNames{i}             = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            soc_dev_abs.IndexNames{i}    = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            soc_dev_p.IndexNames{i}      = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            soc_dev_n.IndexNames{i}      = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            
            %                 P_ch_k0.IndexNames{i}        = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            %                 P_dis_k0.IndexNames{i}       = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            %                 bin_dis_k0.IndexNames{i}     = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            P_gt_k0.IndexNames{i}            = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
            bin_strUP_gt_k0.IndexNames{i}    = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
            bin_shtDOWN_gt_k0.IndexNames{i}  = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
            
        elseif i==2
            pwl_fuel_w.IndexNames{i}     = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            pwl_fuel_bin.IndexNames{i}   = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            P_gt.IndexNames{i}           = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            bin_gt.IndexNames{i}         = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            bin_strUP_gt.IndexNames{i}   = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            bin_shtDOWN_gt.IndexNames{i} = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            pwl_deg_w.IndexNames{i}      = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            pwl_deg_bin.IndexNames{i}    = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            deg.IndexNames{i}            = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            DOD.IndexNames{i}            = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            deg_cyc.IndexNames{i}        = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            bin_dis.IndexNames{i}        = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            P_ch.IndexNames{i}           = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            P_dis.IndexNames{i}          = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            P_d.IndexNames{i}            = indexNames(1,par.N_prd+1:par.N_prd + par.N_scn);
            
            %                 P_gt_k0.IndexNames{i}            = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
            %                 bin_strUP_gt_k0.IndexNames{i}    = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
            %                 bin_shtDOWN_gt_k0.IndexNames{i}  = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
        elseif i==3
            pwl_fuel_w.IndexNames{i}     = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
            pwl_fuel_bin.IndexNames{i}   = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
            P_gt.IndexNames{i}           = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
            bin_gt.IndexNames{i}         = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
            bin_strUP_gt.IndexNames{i}   = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
            bin_shtDOWN_gt.IndexNames{i} = indexNames(1,par.N_prd + par.N_scn+1:par.N_prd + par.N_scn + par.N_gt);
            
            pwl_deg_w.IndexNames{i}      = indexNames(1,par.N_prd + par.N_scn + par.N_gt+1:end);
            pwl_deg_bin.IndexNames{i}    = indexNames(1,par.N_prd + par.N_scn + par.N_gt+1:end);
        else
            pwl_fuel_w.IndexNames{i}     = indexNames(1,par.N_prd + par.N_scn + par.N_gt+1:end);
            pwl_fuel_bin.IndexNames{i}   = indexNames(1,par.N_prd + par.N_scn + par.N_gt+1:end);
        end
    end
    %}
    ...
    %
    tic;
    C_fuel     = optimexpr(par.N_scn);
    C_deg      = optimexpr(par.N_scn);
    C_gt_strUP = optimexpr(par.N_scn);
    C_soc_dev  = optimexpr(par.N_scn);
    C_dump     = optimexpr(par.N_scn);
    y_w        = optimexpr(par.N_scn);
    fuel       = optimexpr(par.N_prd,par.N_scn,par.N_gt);
    C_gt_ON    = optimexpr(par.N_scn);
    ...
    TotFuelCost       = (1/par.N_scn) * sum(y_w);
    probMPC.Objective = TotFuelCost;
    toc
    %}
    %% ------------------\\ Optimization Constraints \\------------------------
    tic;
    sumWeigthsFuelCnstr    = optimconstr(par.N_prd,par.N_scn,par.N_gt);
    contiguityFuelCnstr    = optimconstr(par.N_prd,par.N_scn,par.N_gt,par.N_pwl);
    binIndicatorsFuelCnstr = optimconstr(par.N_prd,par.N_scn,par.N_gt);
    strtUPCnstr            = optimconstr(par.N_prd,par.N_scn,par.N_gt);
    shtDOWNCnstr           = optimconstr(par.N_prd,par.N_scn,par.N_gt);
    shtDOWNCnstr2          = optimconstr(par.N_prd,par.N_scn,par.N_gt);
    rampUpPowGTCnstr       = optimconstr(par.N_prd,par.N_scn,par.N_gt);
    rampDownPowGTCnstr     = optimconstr(par.N_prd,par.N_scn,par.N_gt);
    PgtCnstr               = optimconstr(par.N_prd,par.N_scn,par.N_gt);
    statesGTCnstr          = optimconstr(par.N_prd,par.N_scn,par.N_gt);
    maxPowGTCnstr          = optimconstr(par.N_prd,par.N_scn,par.N_gt);
    minPowGTCnstr          = optimconstr(par.N_prd,par.N_scn,par.N_gt);
    sumWeigthsDegCnstr     = optimconstr(par.N_prd,par.N_scn);
    contiguityDegCnstr     = optimconstr(par.N_prd,par.N_scn,par.N_pwl);
    binIndicatorsDegCnstr  = optimconstr(par.N_prd,par.N_scn);
    degradCnstr            = optimconstr(par.N_prd,par.N_scn);
    DoDCnstr               = optimconstr(par.N_prd,par.N_scn);
    absCyclesPosCnstr      = optimconstr(par.N_prd,par.N_scn);
    absCyclesNegCnstr      = optimconstr(par.N_prd,par.N_scn);
    batBoxChCnstr          = optimconstr(par.N_prd,par.N_scn);
    batBoxDisCnstr         = optimconstr(par.N_prd,par.N_scn);
    powBalanceCnstr        = optimconstr(par.N_prd,par.N_scn);
    %         absSoCDevCnstr1        = optimconstr(par.N_scn);
    %         absSoCDevCnstr2        = optimconstr(par.N_scn);
    TotCycDegradCnstr      = optimconstr(par.N_scn);
    %         TotCalDegradCnstr      = optimconstr(par.N_scn);
    maxCycDegradCnstr      = optimconstr(par.N_scn);
    %         maxCalDegradCnstr      = optimconstr(par.N_scn);
    ...
    for w = 1 : par.N_scn
        ...
        for prdIndx = 0 : par.N_prd-1
            ...
            for g = 1 : par.N_gt
                ...
                contiguityFuelCnstr(prdIndx+1,w,g,par.N_pwl) = pwl_fuel_w(index_k,index_w,index_g,append('point_',int2str(par.N_pwl))) <= ...
                    pwl_fuel_bin(index_k,index_w,index_g,append('point_',int2str(par.N_pwl-1))) ;
            ...
            contiguityDegCnstr(prdIndx+1,w,par.N_pwl) = pwl_deg_w(index_k,index_w,append('point_',int2str(par.N_pwl))) <= ...
                pwl_deg_bin(index_k,index_w,append('point_',int2str(par.N_pwl-1))) ;
        ...
        %             TotCalDegradCnstr(w) = DP_cal(w) == 1 / (par.batLifetime*2) / par.daysOfYear / par.hoursOfday / par.qrtrOfHour * par.N_prd;
    ...
    %         absSoCDevCnstr2 =  1 - DOD_vec(par.N_prd:par.N_prd:par.N_prd*par.N_scn,:) - par.SoC_ref*ones(par.N_scn,1) == soc_dev_p(:) - soc_dev_n(:);
    
    absSoCDevCnstr1 = soc_dev_abs(:)  == soc_dev_p(:) + soc_dev_n(:);
    absSoCDevCnstr2 =  1 - DOD_vec(1:par.N_prd:par.N_prd*par.N_scn,:) - par.SoC_ref*ones(par.N_scn,1) == soc_dev_p(:) - soc_dev_n(:);
    
    % Possible additional contraints for different stochastic policies
    % 2)Same control accross scenarios
    %{
        chargePowAccrosWCnstr    = optimconstr(par.N_prd,par.N_scn^2);
        dischargePowAccrosWCnstr = optimconstr(par.N_prd,par.N_scn^2);
        strtUPindicAccrosWCnstr  = optimconstr(par.N_prd,par.N_scn^2);
        shutOFFindicAccrosWCnstr = optimconstr(par.N_prd,par.N_scn^2);
    ...
    %{
        chargePowAccrosW1Cnstr    = optimconstr(par.N_scn-1);
        dischargePowAccrosW1Cnstr = optimconstr(par.N_scn-1);
        disIndindicAccrosW1Cnstr  = optimconstr(par.N_scn-1);
        strtUPindicAccrosW1Cnstr  = optimconstr(par.N_scn-1,par.N_gt);
        shutOFFindicAccrosW1Cnstr = optimconstr(par.N_scn-1,par.N_gt);
        gtPwrAccrosW1Cnstr        = optimconstr(par.N_scn-1,par.N_gt);
    ...
    %
    chargePowAccrosK0Cnstr    = optimconstr(par.N_scn);
    dischargePowAccrosK0Cnstr = optimconstr(par.N_scn);
    disIndindicAccrosK0Cnstr  = optimconstr(par.N_scn);
    strtUPindicAccrosK0Cnstr  = optimconstr(par.N_scn,par.N_gt);
    shutOFFindicAccrosK0Cnstr = optimconstr(par.N_scn,par.N_gt);
    gtPwrAccrosK0Cnstr        = optimconstr(par.N_scn,par.N_gt);
    ...
    % Final state of charge penalization
    %         absSoCDevCnstr1(w) = soc_dev_abs(index_w)  == soc_dev_p(index_w) + soc_dev_n(index_w);
    %         absSoCDevCnstr2(w) =  1 - DOD(append('k+',int2str(par.N_prd-1)),index_w) - par.SoC_ref == soc_dev_p(index_w) - soc_dev_n(index_w);
