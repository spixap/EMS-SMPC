function [xi] = funScenGenQRF(ttData, par, Data, t_current, Mdl, animPar, fig_ctrl)
...
%         FigH = figure;
    ...
    for idx_t = t_current : t_final
        ...
        %% Recursively updated Sigma (initialization at the first simulation timestep) - Version 1
        %{
        if idx_gif >= par.N_prd + 1
            X = zeros(par.N_prd,1);
            for k = 1 : par.N_prd
                X(k,1) = X_k(idx_gif-par.N_prd,k);
            end
            Sigma_hat_rec_t_inf  = par.lamda * Sigma_rec_t_prev_inf + (1-par.lamda) * (X*X');
            Sigma_rec_t_prev_inf = Sigma_hat_rec_t_inf;
        else
            Sigma_rec_t_prev_inf = diag(ones(par.N_prd,1));
            Sigma_hat_rec_t_inf = Sigma_rec_t_prev_inf;
        end
        Rho_hat_rec_t_inf = corrcov(Sigma_hat_rec_t_inf);
        %}
        %% Recursively updated Sigma (initialization at the first simulation timestep) - Version 2
        %
        if idx_gif == 1
            Sigma_rec_t_prev_inf = diag(ones(par.N_prd,1));
            Sigma_hat_rec_t_inf = Sigma_rec_t_prev_inf;
...
%         end
        ...
        xi.Sigma(idx_gif,:,:) = Sigma_hat_rec_t_inf;
        xi.Rho(idx_gif,:,:)   = Rho_hat_rec_t_inf;
